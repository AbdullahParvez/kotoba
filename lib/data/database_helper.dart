...
Future<dynamic> getKanji(kanji) async {
  final db = await _getDatabase();
  List<Map<String, dynamic>> details =
      await db.query("kanji", where: "kanji = ?", whereArgs: [kanji.trim()]);
  if (details.isNotEmpty){
    return Kanji(
      id: details[0]['kanjiId'],
      kanji: details[0]['kanji'],
      kunyomi: details[0]['kunyomi'],
      onyomi: details[0]['onyomi'],
      meaning: details[0]['meaning'],
    );
  }
}
